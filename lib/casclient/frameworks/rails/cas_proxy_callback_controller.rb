...
class CasProxyCallbackController < ActionController::Base
  ...
  def receive_pgt
    #FIXME: these checks don't work because REMOTE_HOST doesn't work consistently under all web servers (for example
    # it doesn't work at all under mongrel) ... need to find a reliable way to check if the request came through from a
    # reverse HTTPS proxy -- until then I'm disabling this check
